...
def retransform_(
    ...
    for i in range(N):
        ...
        if formula == "lin":  # Levels (No Transformation)
            ...
            # V[0:T, i] = np.cumsum(X[c_idx[0]:H, i]) + Z[c_idx[0] - step, i]  # Assuming X[t1-step] is a last historical value
            V[0:T, i] = np.add(X[c_idx[0]:H, i], Z[c_idx[0] - step:H-step, i])
        elif formula == "ch1":
            V[0:T, i] = np.add(Z[c_idx[0]-12:H-12, i], X[c_idx[0]:H, i])
        elif formula == "pch":  # Percent Change
            # V[0:T, i] = np.cumprod(1 + (X[c_idx[0]:H, i] / 100)) * Z[c_idx[0] - step, i]  # Assuming X[t1-step] is a last historical value
            V[0:T, i] = np.multiply(1 + (X[c_idx[0]:H, i] / 100), Z[c_idx[0] - step:H-step, i])
        elif formula == "pc1":  # Year over Year Percent Change
            V[0:T, i] = np.multiply(
                1 + (X[c_idx[0]:H, i] / 100), Z[c_idx[0]-12:H-12, i]
            )
        elif formula == "pca":  # Percent Change (Annual Rate)
            # V[0:T, i] = np.cumprod((1 + X[c_idx[0]:H, i] / 100) ** n) * Z[c_idx[0] - step, i]
            V[0:T, i] = ((1 + X[c_idx[0]:H, i] / 100) ** n) * Z[c_idx[0] - step:H-step, i]            
